...
from ui_sidebar import (
    render_settings_card, render_expected_move_panel, render_key_levels,
    render_wall_credibility, render_gex_stream, render_data_quality,
)
from ui_theme import (
    inject_global_css, get_ui_state, qlink, map_exp_mode, map_refresh,
    render_header, fmt_commas, esc, TABS, QUICK_TICKERS,
)
...
def main():
    ...
    # ── UI state (URL query params drive all navigable controls) ──
    ui_state = get_ui_state()

    # Transient refresh action: clear the cache, drop the param, rerun clean.
    if st.query_params.get("refresh_now"):
        st.cache_resource.clear()
        try:
            del st.query_params["refresh_now"]
        except KeyError:
            pass

    mode = map_exp_mode(ui_state["exp"])
    mode_short = mode
    refresh_option = map_refresh(ui_state["refresh"])
    tab = ui_state["tab"]

    # ── Resolve active ticker (validate non-curated typed symbols) ──
    from phase1.ticker_config import all_tickers, get_config
    _curated = all_tickers()
    st.session_state.setdefault("ticker_meta", {})

    ticker = (ui_state["ticker"] or "SPX").strip().upper()
    ticker_error = None
    ...
    # quick picks which already have their own chip row. Carried in the URL
    # (``?recents=…``) — NOT st.session_state — because every anchor click is a
    # full page reload, which resets session_state but preserves query params.
    RECENTS_MAX = 6
    prior = [t.strip().upper() for t in ui_state.get("recents", "").split(",") if t.strip()]
    recents = [t for t in prior if t not in QUICK_TICKERS]
    if ticker not in QUICK_TICKERS:  # a valid custom symbol (SPX-fallback is curated)
        recents = [ticker] + [t for t in recents if t != ticker]
    recents = recents[:RECENTS_MAX]
    # Write the resolved list back into ui_state so every qlink()/hidden-input
    # built below carries it forward through the next navigation.
    ui_state["recents"] = ",".join(recents)
    ...
    else:  # Custom — date range from the calendar (query params)
        _today = run_now.date()
    ...
    header_html = render_header(
        ...
        clock=data.run_time, refresh_href=qlink(ui_state, refresh_now="1"),
        live=is_market_open,
    )
    st.html(header_html)
    ...
    # ── Aside (sidebar cards) ──
    aside = (
        render_settings_card(ui_state, ticker, ticker_type, recents, future_exps)
    ...
    tab_bar = '<div class="tab-bar">' + "".join(
        f'<a class="tab{" on" if tok == tab else ""}" target="_self" href="{qlink(ui_state, tab=tok)}">{esc(label)}</a>'
        for tok, label in TABS
    ) + '</div>'
    ...
    if tab == "gex":
        ...
        label = "🎯 Spread Finder" if tab == "spread" else "⚡ 0DTE Finder"
        tab_content = (
            '<section class="gex-wrap" style="min-height:auto;">'
            f'<div style="font-size:13px;color:var(--text-secondary);font-weight:700;">{esc(label)}</div>'
            '<div style="font-size:11px;color:var(--text-dim);margin-top:6px;">'
            'Terminal styling lands in Phase 2 — the working view renders below for now.</div></section>'
        )

    body_html = (
        '<div class="term-body">'
        f'<aside class="term-aside">{aside}</aside>'
        f'<main class="term-main">{em_strip}{tab_bar}{tab_content}</main>'
        '</div>'
    )
    st.html(body_html)

    # ── Phase-1 interim: render the (un-restyled) finder tabs below the shell ──
    if tab == "spread":
