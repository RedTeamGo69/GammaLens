...
from ui_theme import (
    esc, fmt_commas, qlink, QUICK_TICKERS, EXP_MODES, REFRESH_MODES,
)
...
# ─────────────────────────────────────────────────────────────────────────────
# Settings card (interactive: ticker / expiration / auto-refresh / calendar)
# ─────────────────────────────────────────────────────────────────────────────
def render_settings_card(ui_state: dict, ticker: str, ticker_type: str,
                         recents: list[str], future_exps: list[str]) -> str:
    """Instrument + Expiration + Auto-refresh controls as anchor/form HTML."""
    # ── ticker search form (GET → ?ticker=…) preserving other state ──
    hidden = "".join(
        f'<input type="hidden" name="{k}" value="{esc(v)}">'
        for k, v in ui_state.items()
        if k not in ("ticker", "cal_start", "cal_end", "cal_offset") and v
    )
    search = (
    ...
    # ── ticker chips ──
    def _chip(sym: str) -> str:
        return (f'<a class="chip{" on" if sym == ticker else ""}" target="_self" '
                f'href="{qlink(ui_state, ticker=sym)}">{esc(sym)}</a>')

    chips = "".join(_chip(sym) for sym in QUICK_TICKERS)

    # ── recent chips (own labelled section; quick picks already shown above) ──
    recent_syms = [t for t in recents if t not in QUICK_TICKERS]
    recent_block = ""
    if recent_syms:
        recent_chips = "".join(_chip(sym) for sym in recent_syms)
        recent_block = (
            '<div class="card-eyebrow" style="margin:10px 0 8px;">Recent</div>'
            f'<div class="chip-wrap">{recent_chips}</div>'
        )

    # ── expiration chips ──
    exp_tok = ui_state.get("exp", "0dte")
    exp_chips = "".join(
        f'<a class="chip{" on" if tok == exp_tok else ""}" target="_self" '
        f'href="{qlink(ui_state, exp=tok)}">{esc(label)}</a>'
        for tok, label in EXP_MODES
    )

    calendar = _render_calendar(ui_state, future_exps) if exp_tok == "custom" else ""

    # ── auto-refresh segmented control ──
    ref_tok = ui_state.get("refresh", "off")
    ref_segs = "".join(
        f'<a class="seg{" on" if tok == ref_tok else ""}" target="_self" '
        f'href="{qlink(ui_state, refresh=tok)}">{esc(label)}</a>'
        for tok, label in REFRESH_MODES
    )

    return f"""
<section class="term-card">
  <div class="card-eyebrow">Instrument</div>
  {search}
  <div class="chip-wrap" style="margin-bottom:8px;">{chips}</div>
  {recent_block}
  <div style="font-size:10px;color:var(--text-dim);margin:11px 0 13px;">Active:
    <span style="font-family:var(--mono);color:var(--text-secondary);font-weight:600;">{esc(ticker)}</span>
    <span style="color:var(--text-dim);">· {esc(ticker_type)}</span></div>
  <div class="card-eyebrow" style="margin-bottom:9px;">Expiration</div>
  <div class="chip-wrap">{exp_chips}</div>
  {calendar}
  <div class="card-eyebrow" style="margin:14px 0 9px;">Auto-refresh</div>
  <div class="seg-wrap">{ref_segs}</div>
</section>
"""


def _render_calendar(ui_state: dict, future_exps: list[str]) -> str:
    """Custom date-range calendar popover (two-click range via anchors)."""
    try:
        offset = int(ui_state.get("cal_offset", "0") or "0")
    except ValueError:
        offset = 0
    offset = max(0, min(3, offset))
    cal_start = ui_state.get("cal_start", "") or ""
    cal_end = ui_state.get("cal_end", "") or ""
    exp_set = set(future_exps or [])

    today = date.today()
    disp_year = today.year + (today.month - 1 + offset) // 12
    disp_month = (today.month - 1 + offset) % 12 + 1
    first = date(disp_year, disp_month, 1)
    month_names = ["January", "February", "March", "April", "May", "June", "July",
                   "August", "September", "October", "November", "December"]
    title = f"{month_names[disp_month - 1]} {disp_year}"

    start_dow = (first.weekday() + 1) % 7  # python Mon=0 → grid Sun=0
    days_in_month = (date(disp_year + (disp_month // 12), (disp_month % 12) + 1, 1) - timedelta(days=1)).day

    cells: list[date | None] = [None] * start_dow
    cells += [date(disp_year, disp_month, d) for d in range(1, days_in_month + 1)]
    while len(cells) % 7:
        cells.append(None)

    def day_cell(dt: date | None) -> str:
        if dt is None:
            return '<div style="visibility:hidden;padding:6px 0;"></div>'
        iso = dt.isoformat()
        dow = (dt.weekday() + 1) % 7
        is_past = iso < today.isoformat()
        is_exp = (1 <= dow <= 5) and not is_past and (not exp_set or iso in exp_set)
        is_start = iso == cal_start
        is_end = iso == cal_end
        is_edge = is_start or is_end
        in_range = bool(cal_start) and bool(cal_end) and cal_start <= iso <= cal_end
        base = ("font-family:var(--mono);font-size:11px;padding:6px 0;border-radius:6px;"
                "text-align:center;border:1px solid transparent;display:block;text-decoration:none;")
        if is_past:
            style = base + "color:#39424f;cursor:default;background:transparent;"
            return f'<div style="{style}">{dt.day}</div>'
        # compute resulting range for this click (mirror onDayClick)
        if (not cal_start) or (cal_start and cal_end):
            n_start, n_end = iso, ""
        elif iso < cal_start:
            n_start, n_end = iso, ""
        else:
            n_start, n_end = cal_start, iso
        href = qlink(ui_state, cal_start=n_start, cal_end=(n_end or None))
        if is_edge:
            style = base + ("color:#06140c;background:var(--green);cursor:pointer;font-weight:700;"
                            "box-shadow:0 0 8px rgba(43,232,138,.4);")
        elif in_range:
            style = base + "color:var(--text-secondary);background:rgba(43,232,138,.14);cursor:pointer;"
        else:
            ul = "border-bottom:2px solid rgba(43,232,138,.5);" if is_exp else ""
            col = "var(--text-secondary)" if is_exp else "var(--text-dim)"
            style = base + f"color:{col};background:transparent;cursor:pointer;{ul}"
        return f'<a target="_self" href="{href}" style="{style}">{dt.day}</a>'

    weeks = ""
    for i in range(0, len(cells), 7):
        row = "".join(day_cell(c) for c in cells[i:i + 7])
        weeks += f'<div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;">{row}</div>'

    # nav + footer
    prev_off = max(0, offset - 1)
    next_off = min(3, offset + 1)
    nav_base = ("font-family:var(--mono);font-size:15px;line-height:1;width:24px;height:24px;"
                "border-radius:6px;background:var(--bg-row);border:1px solid var(--border-mid);"
                "text-decoration:none;display:flex;align-items:center;justify-content:center;")
    prev_btn = (f'<a target="_self" href="{qlink(ui_state, cal_offset=prev_off)}" '
                f'style="{nav_base}color:{"var(--text-secondary)" if offset > 0 else "#39424f"};">‹</a>')
    next_btn = (f'<a target="_self" href="{qlink(ui_state, cal_offset=next_off)}" '
                f'style="{nav_base}color:{"var(--text-secondary)" if offset < 3 else "#39424f"};">›</a>')

    if not cal_start:
        caption = "Pick a start date"
    elif not cal_end:
        caption = f"{cal_start} → pick end"
    else:
        cnt = len([e for e in exp_set if cal_start <= e <= cal_end]) if exp_set else 0
        caption = f"{cnt} expiration{'' if cnt == 1 else 's'} · {cal_start} → {cal_end}"
    clear_btn = (f'<a target="_self" href="{qlink(ui_state, cal_start=None, cal_end=None)}" '
                 'style="font-family:var(--sans);font-size:10px;font-weight:600;color:var(--text-muted);'
                 'background:var(--bg-row);border:1px solid var(--border-mid);padding:4px 10px;'
                 'border-radius:6px;text-decoration:none;">Clear</a>')

    weekday_hdr = "".join(
        f'<div style="text-align:center;font-size:9px;font-weight:700;color:var(--text-dim);'
        f'letter-spacing:.03em;padding:2px 0;">{d}</div>'
        for d in ["S", "M", "T", "W", "T", "F", "S"]
    )

    return f"""
<div style="margin-top:11px;background:var(--bg-input);border:1px solid var(--border-mid);border-radius:9px;padding:11px;">
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:9px;">
    {prev_btn}
    <span style="font-family:var(--sans);font-size:12px;font-weight:700;color:var(--text-secondary);letter-spacing:.02em;">{esc(title)}</span>
    {next_btn}
  </div>
  <div style="display:grid;grid-template-columns:repeat(7,1fr);gap:2px;margin-bottom:4px;">{weekday_hdr}</div>
  <div style="display:flex;flex-direction:column;gap:2px;">{weeks}</div>
  <div style="display:flex;align-items:center;justify-content:space-between;margin-top:10px;padding-top:9px;border-top:1px solid var(--border);">
    <span style="font-size:10px;color:var(--text-muted);font-family:var(--mono);">{esc(caption)}</span>
    {clear_btn}
  </div>
</div>
"""
