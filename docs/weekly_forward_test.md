...
The implementation is observational and contains no order submission or execution accounting. This runbook describes its behavior and release sequence. The [September 8 source review](weekly_forward_test_history_resolution.md) records the current history policy and pre-release evidence; the original implementation/review documents retain their historical status.
...
- Opening capture: about **20 Tradier GETs** (two histories, one batch quote, expirations and one chain per ticker), **4 Yahoo corroborating history GETs**, **3 Cboe history requests** and **3 FRED series requests**. Common supplemental inputs are reused across the four tickers in one invocation. Failed attempts are capped at three per model; the study's Tradier history/quote/timesales requests have 20-second timeouts and at most two HTTP retries. Yahoo uses one GET with a 20-second timeout and no HTTP retry loop. Existing chain/source clients are reused; the overall GitHub job has a 25-minute limit.
