...
The implementation is observational and contains no order submission or execution accounting. This runbook describes its behavior and release sequence. The [September 8 source review](weekly_forward_test_history_resolution.md) records the former corroboration policy and its pre-release evidence. As of the September 25 revision, new captures use Tradier history directly; earlier evidence and forecasts retain their original source policy.
...
The active study is **`spread-finder-weekly-v3-open`**, starting **September 28, 2026**.
...
time, including an empty new study before its first capture. The unused v2 registration is retained as a superseded registration; v3 records
the user-approved opening-session policy before its first capture. A restart does not
...
The underlying's first-session daily open and VIX open anchor the range. Quote identity, session date, positive prices and freshness are checked. Freshness allows the declared feed delay (0 or 900 seconds) plus 120 seconds; the default is real time. Tradier supplies historical weekly/daily prices directly, without Yahoo requests, corroboration, or replacement bars. The `tradier-primary-history-v2` policy retains raw Tradier receipts and prices, validates weekly OHLC and the daily closes consumed by training, and checks session coverage and daily/weekly final-close consistency. The old review catalog is retained for historical reproduction only and is not used for new captures. Cboe supplies volatility history, the existing FRED collector supplies macro history and existing calendars supply known events. No primary yfinance path was added. Legacy weekly storage determines the full older horizon for side-share estimation and HAR lags; fresh validated bars cover that horizon without overwriting legacy history. The original feature arithmetic is shared through an injected, non-persisting `build_features` path. Interactive Spread Finder retains its existing sources and can therefore show different recommendations.

The target week's outcomes and later rows cannot enter training. Daily exchange-session gaps, missing prior-week bars, stale prior-final-session VIX history, duplicate labels, invalid weekly OHLC and invalid daily closes are rejected. Daily training consumes only closes; unused daily OHL cannot veto training. Outcome observations continue to require valid full OHLC. M4 detects stale/missing term-structure or macro sources. Stored data are the source snapshots available when this forecast was generated; subsequent historical refreshes cannot replace them. Calendar event coverage must extend through the evaluation week. When the existing event lists expire, extend those calendars through their normal maintenance process; forecasts fail explicitly until coverage is restored.
...
- Capture opens **at the first actual session open** and ends **45 minutes after that open, exclusive**: normally **09:30 <= availability < 10:15 ET**.
- Scheduled attempts are 09:30, 09:40 and 09:50 ET on weekdays. Only the week's first actual session admits a primary capture.
...
For example, the week labeled 2026-09-07 opens on Tuesday 2026-09-08. The capture window is Tuesday 09:30–10:15 ET. Thanksgiving week's Friday evaluation uses the calendar's 13:00 close; a Friday-holiday week evaluates on its actual last session.
...
The original pre-activation review is in [weekly_forward_test_release_review.md](weekly_forward_test_release_review.md), followed by the [source resolution](weekly_forward_test_history_resolution.md). Keep `FORWARD_TEST_ENABLED=false` until all release gates below pass. Read-only prerequisites can be repeated with `scripts/check_forward_test_prerequisites.py --secrets-file .streamlit/secrets.toml --output <local-evidence.json>` using the project interpreter. This command never fits models, writes forecasts or migrates tables. Midweek checks clearly distinguish available completed history from a still-future prior Friday required at capture.
...
7. After the first eligible opening run, check the dashboard and workflow health: 28 captured models / 112 tiers, or explicit unavailable/missed reasons. After the final-session collection, check independent close and path sample counts. New opening-session forecasts use complete daily OHLC for path coverage; archived partial-session forecasts still require minute history. Verify the real account's SPX, quote and history availability during this first run; no live production certification was attempted here.
