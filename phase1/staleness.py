...
def build_staleness_info(calendar_snapshot: dict, spot_info: dict, stats: dict, has_0dte: bool = False) -> dict:
    ...
    # Forced or degraded broker-quote spot usage
    if spot_source.startswith("broker (forced, market closed)"):
        score -= 6
        reasons.append("Spot source was forced to the broker quote because the market was closed.")
        defenses_triggered.append("forced_broker_spot")
    elif spot_source.startswith("broker") and market_open:
        score -= 18
        reasons.append("Market open but implied parity did not supply the reference spot.")
        defenses_triggered.append("broker_spot_during_market_hours")
